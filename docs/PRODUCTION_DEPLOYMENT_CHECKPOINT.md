...
**Checkpoint date:** 2026-07-24, continued after a power-outage interruption, same day. **`https://api.loanmanagerapp.com` is now fully live** — DNS resolved, managed SSL certificate `ACTIVE`, Cloud Run public access enabled, and the full LB→Cloud Run→NestJS→Firebase Admin→Cloud SQL path verified end-to-end at the infrastructure level (TLS handshake, correct 404/401 responses, live DB query in logs). Backend revision `loan-manager-backend-00006-2dp`, serving 100% traffic. `CORS_ORIGIN` is live as `https://loanmanagerapp.com`. Release keystore SHA-1/SHA-256 registered in Firebase. A signed production Release APK has been built and its signature verified (`apps/customer-app/build/app/outputs/flutter-apk/app-release.apk`), pointed at `https://api.loanmanagerapp.com/api`. **No app-level functional testing (login/OTP/loan flow/etc.) has been performed** — that remains the user's own manual pass on a physical device. See §8 for full detail, including the mid-provisioning cert issue and its fix.
