...
**Checkpoint date:** 2026-07-24. **`https://api.loanmanagerapp.com` is fully live and the Customer App is FROZEN as of this date.** The user completed a full manual functional testing pass on a physical device against the real production backend (Google Sign-In, Phone OTP, logout/login, Apply Loan, document upload/replace, notifications, profile, internet off/on) — all passed, after three real bugs were found and fixed along the way (see `docs/PRODUCTION_READINESS_CHECKLIST.md` §7 for the full list). Backend revision `loan-manager-backend-00008-hct`, serving 100% traffic, publicly accessible. `CORS_ORIGIN` live as `https://loanmanagerapp.com`. Release keystore SHA-1/SHA-256 registered in Firebase. The signed production Release APK installed and tested on-device is the final, frozen build. **No further Customer App changes without an explicit new decision to unfreeze** — see memory `customer-app-frozen-2026-07-24`. See §8 for full infra detail, including the mid-provisioning cert issue and its fix.
...
**DNS added by user:** A record, host `api`, value `34.111.88.162` — resolved correctly on both Google (8.8.8.8) and Cloudflare (1.1.1.1) resolvers as soon as it was added.

**Cert issue found and fixed:** the original `loan-manager-api-cert` got stuck reporting `domainStatus: FAILED_NOT_VISIBLE` well after DNS was confirmed correctly resolving everywhere. Diagnosed before assuming misconfiguration — checked (all clean): DNS A record, no conflicting/duplicate A or AAAA records, no CAA record restricting Google's CA, DNSSEC validating correctly (`AD:true`). Root cause: the cert's very first Google validation check ran hours before DNS existed (cert was created early, DNS added much later in the session); that stale negative result didn't self-clear on its own schedule within a reasonable window. Fix: created a fresh cert resource (`loan-manager-api-cert-v2`), attached it to the HTTPS proxy alongside the old one (zero downtime), confirmed it validated cleanly (`domainStatus: ACTIVE` within ~8 minutes of creation, no `FAILED_NOT_VISIBLE` detour this time), then removed the old cert from the proxy and deleted it. **The live cert resource is now `loan-manager-api-cert-v2`** (GCP certs can't be renamed in place, so the `-v2` suffix is permanent — not a typo or leftover).

**Verified end-to-end at the infrastructure level (no app UI/functional testing, per explicit instruction):**
- `curl https://api.loanmanagerapp.com/` — valid TLS handshake (`ssl_verify_result:0`), served by Google Frontend, correct NestJS `404` JSON body for an undefined route. Confirms DNS → LB → managed cert → Cloud Run → NestJS all work.
- `GET /api/v1/auth/me` with no token — `401 {"message":"Missing bearer token."}`, the app's own `FirebaseAuthGuard`, not Cloud Run's IAM-level 403. Confirms Firebase Admin initialized correctly and the guard chain is live (a broken Firebase Admin init would not produce this clean a response).
- Cloud Run logs at the moment of testing show a fresh, successful query against the `migrations` table ("No migrations are pending") — live proof of a healthy Cloud SQL connection right now, not just historically.
- Cloud Run public access enabled: `gcloud run services add-iam-policy-binding loan-manager-backend --member=allUsers --role=roles/run.invoker` — deliberately held back until the cert was confirmed working via a real request, then done as its own explicit step.

**Also done this session:** release keystore SHA-1/SHA-256 registered in Firebase for the Android app (`1:660520519709:android:21ebd181c8625fdc8edef4`) — via the Firebase Management API directly (`firebase.googleapis.com/v1beta1/.../androidApps/{appId}/sha`), authenticated with the existing gcloud credentials, no Firebase Console click-through needed. Both fingerprints verified against the actual keystore file (`keytool -list -v`) before submission, and confirmed present afterward via a follow-up GET. A signed production Release APK was built twice (once before the backend was fully reachable, then rebuilt as the definitive final artifact after public access was confirmed) via `flutter build apk --release --dart-define-from-file=env/production.json`, at `apps/customer-app/build/app/outputs/flutter-apk/app-release.apk` (67.9 MB) — signature verified via `apksigner` both times, matching the same SHA-1/SHA-256 now registered in Firebase. It points at `https://api.loanmanagerapp.com/api`, which is now live. No app functionality was tested (explicitly out of scope this session — the user tests manually on a physical device).
