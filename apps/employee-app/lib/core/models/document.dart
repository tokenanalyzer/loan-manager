...
/// mirrors `DocumentTypeOverviewDto`.
...
/// Mirrors `DocumentsOverviewResponseDto` — categories, each with its
/// document types, each with its upload slots.
class DocumentsOverview {
  ...
  /// Flattened uploaded documents, paired with their category/type
  /// label — what staff review actually needs (a list of what was
  /// submitted), rather than the customer-facing "required slots,
  /// including empty ones" view.
  List<(String typeLabel, AppDocument document)> get uploadedDocuments {
    final result = <(String, AppDocument)>[];
    for (final group in categories) {
      for (final type in group.types) {
        for (final slot in type.slots) {
          if (slot.document != null) {
            result.add((type.label, slot.document!));
          }
        }
      }
    }
